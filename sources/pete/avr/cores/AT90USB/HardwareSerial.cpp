...
/*
  Modified April 2013 by Parker Dillmann for RS485
*/
...
#if defined(USBCON)
// hardware dual-banked TX and RX buffers used on USB AVR, no software buffer used
...
void HardwareSerial::begin(unsigned long baud)
{
  ...
  bool use_u2x = true;
  _pin = 0;
  SER_CLEAR_TM = 1000000.0 / baud + 0.07;
#if F_CPU == 16000000UL
...
{
  uint8_t i = (_tx_buffer->head + 1) % SERIAL_BUFFER_SIZE;
...
void HardwareSerial::setPin(int pin) 
{
    _pin = pin;
    pinMode(_pin,OUTPUT);
    digitalWrite(_pin,LOW);
